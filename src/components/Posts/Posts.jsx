...
import {
  nextPageAction,
  viewPostsAction,
} from "../../storage/reducers/paginateReducers";
...
export default function Posts({
  ...
  const allPage = new Array(maxPage)
    .fill(null)
  ...
  const word = (length, w) => {
    if (length % 10 === 1) {
      ...
      return w + "а";
    } else if (length % 10 >= 5 || length % 10 < 1 || !posts.length) {
      return w + "ов";
    }
  };
      ...
      <div className={s.posts}>
        <div className={s.container}>
          ...
          {searchQuery && (
            <div className={s.notification}>
              <UpperNotific
                message={
                  posts.length > 0
                    ? `Найдено ${posts.length} ${word(posts.length, "пост")}`
                    : `Посты не найдены`
                }
                visible={activeSearch}
                setVisible={setActiveSearch}
              />
            </div>
          )}
          ...
          <div className={s.inner}>
            {isLoading ? (
              postsForPaginate.map((item, index) => (
                <Post
                  key={item._id}
                  post={item}
            ...
            !urlOutPaginate.some((path) =>
              ...
              <div>
                ...
                <div className={s.amount}>
                  ...
                  <select
                    value={viewPosts}
                    onChange={(e) => amountPosts(Number(e.target.value))}
                  >
